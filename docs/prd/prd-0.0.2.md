# ReviewDesk egui UI Redesign PRD 0.0.2
...
- 이전 UI PRD: `docs/prd/prd-0.0.1.md`
- 0.0.2 목적: 에이전트 검증을 통과한 UI 리디자인 요구사항을 다음 구현 대상 버전으로 확정

## 1.1 0.0.2 변경 요약

0.0.2는 0.0.1 UI PRD의 검증 통과본을 다음 구현 기준으로 승격한 버전이다.

변경 사항:

- 문서 버전을 `0.0.2`로 승격했다.
- 상태를 `구현 계획 작성 가능`으로 정리했다.
- 0.0.1 문서를 이전 UI PRD로 보존했다.
- Product/UX, Rust egui Architecture, Visual/Accessibility, QA/Acceptance 2차 검증 PASS 결과를 유지했다.
- UI 구현 범위는 3-pane egui 데스크톱 리디자인으로 고정한다.
...
0.0.2 UI 리디자인은 Rust `egui`/`eframe` 코드에 반영되었다.

구현된 범위:

- Top Bar: GitHub 상태, AI 상태, model selector, reasoning depth selector, refresh, task status
- Left Queue Pane: repository selector, review requested/assigned filter, PR row list, direct PR fallback
- Center PR Workspace: PR header, risk/CI/status summary, Overview/Findings/Files/Checklist tabs, AI blocked banner, private diff consent banner
- Right Draft Pane: review event selector, editable draft editor, safety checklist, save/copy/submit actions, disabled reason
- Bottom Status Strip: last task/result status
- Submit confirmation dialog: repository, PR, event, head SHA, stale check 표시
- Wide 3-pane layout and narrow `Queue / Review / Draft` tabbed layout
- UI view state helpers and tests for disabled reason, queue filter, direct PR validation, dialog state, snapshot names, viewport mode
...
- `docs/verification/reviewdesk-egui-ui-0.0.2.md`
...
- 실제 GitHub OAuth runtime wiring과 실제 ChatGPT OAuth provider invocation은 0.0.2 UI PRD의 제외 범위이므로 이번 UI 구현에는 포함하지 않았다.
- UI는 local fixture state로 주요 상태와 상호작용을 보여주며, 기존 GitHub/API core 동작은 별도 테스트에서 검증한다.
